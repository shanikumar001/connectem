...
router.put('/', auth, async (req, res) => {
  try {
    ...
    if (user.role === 'mentor' && mentorProfile) {
      ...
      if (allowedMentor.fullName && allowedMentor.expertise && allowedMentor.bio) {
      ...
      // Mark as completed if mandatory fields are present
      if (allowedCompany.companyName && allowedCompany.industry && allowedCompany.description) {
        user.isProfileCompleted = true;
      }
    } else {
...
router.get('/complete', auth, async (req, res) => {
  try {
    res.json({ isComplete: !!req.user.isProfileCompleted });
  } catch (error) {
